...
燕返采用 Rust 实现，裁剪到实际需要的功能，UPX 压缩后约 700KB。它不追求大而全，只覆盖日常分流场景中最常用的那部分：

- **GeoIP 分流**：根据 IP 归属地自动选择国内/国外上游
- **GFWList 匹配**：布隆过滤器实现，内存占用小
- **DNS 污染检测**：识别并过滤 GFW 注入的伪造响应
- **广告屏蔽**：支持 AdBlock Plus 规则格式
- **本地域名转发**：`.lan`、`.home` 等后缀走指定上游
- **DNS 缓存**：LRU 缓存，TTL 自动过期
- **Hosts 覆盖**：支持多 IP 负载均衡
- **nftables 联动**：解析到的 IP 自动加入防火墙 set

> **为什么不用 geosite？** geosite 规则集包含数万条域名，加载到内存后占用可达数十 MB，对 flash 和 RAM 都紧张的小路由不友好。燕返选择 GFWList（布隆过滤器，内存占用可控）+ GeoIP 判断（数据库按需查询）的组合，实际分流效果相当，内存开销却小得多。
...
special_upstream    = "127.0.0.1:53"     # 本地 dnsmasq（保持 53 端口）
...
```bash
uci set tsubamegaeshi-rs.service.enabled=1
/etc/init.d/tsubamegaeshi-rs start
```

启动时自动完成：dnsmasq 上游指向 `127.0.0.1#5353`，关闭 dnsmasq 缓存和外部 DNS；停止时自动恢复原配置。

如需强制劫持局域网中绕过 dnsmasq 的设备（如手动设了 DNS 的手机/电脑）：

```bash
uci set tsubamegaeshi-rs.service.dns_hijack=1
/etc/init.d/tsubamegaeshi-rs restart
```
...
# ---------- 基本 ----------
listen = "0.0.0.0:5353"               # 【必填】监听地址，也支持 "[::]:5353"（双栈）

# ---------- 上游服务器 ----------【均必填】
# 支持 "ip"、"ip:port"、"[v6]:port"、裸 IPv6 四种写法，省略端口时默认 53
special_upstream   = "127.0.0.1:53"     # 通常指向本地 dnsmasq（保持 53 端口）
domestic_upstream  = "223.5.5.5"
foreign_upstream   = "8.8.8.8"
